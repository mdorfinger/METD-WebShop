 ...
 * @method productsQuery orderByBeschr($order = Criteria::ASC) Order by the beschr column
 *
 * @method productsQuery groupById() Group by the id column
 * @method productsQuery groupByBeschr() Group by the beschr column
 *
 ...
 *
 * @method productsQuery leftJoincart($relationAlias = null) Adds a LEFT JOIN clause to the query using the cart relation
 * @method productsQuery rightJoincart($relationAlias = null) Adds a RIGHT JOIN clause to the query using the cart relation
 * @method productsQuery innerJoincart($relationAlias = null) Adds a INNER JOIN clause to the query using the cart relation
 *
 ...
 *
 * @method products findOneByBeschr(string $beschr) Return the first products filtered by the beschr column
 *
 * @method array findById(int $id) Return products objects filtered by the id column
 * @method array findByBeschr(string $beschr) Return products objects filtered by the beschr column
 *
...
abstract class BaseproductsQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT `id`, `beschr` FROM `products` WHERE `id` = :p0';
        try {
    ...
    /**
     * Filter the query on the beschr column
     *
     * Example usage:
     * <code>
     * $query->filterByBeschr('fooValue');   // WHERE beschr = 'fooValue'
     * $query->filterByBeschr('%fooValue%'); // WHERE beschr LIKE '%fooValue%'
     * </code>
     *
     * @param     string $beschr The value to use as filter.
     *              Accepts wildcards (* and % trigger a LIKE)
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return productsQuery The current query, for fluid interface
     */
    public function filterByBeschr($beschr = null, $comparison = null)
    {
        if (null === $comparison) {
            if (is_array($beschr)) {
                $comparison = Criteria::IN;
            } elseif (preg_match('/[\%\*]/', $beschr)) {
                $beschr = str_replace('*', '%', $beschr);
                $comparison = Criteria::LIKE;
            }
        }

        return $this->addUsingAlias(productsPeer::BESCHR, $beschr, $comparison);
    }
    ...
    public function filterBycart($cart, $comparison = null)
    {
        if ($cart instanceof cart) {
            return $this
                ->addUsingAlias(productsPeer::ID, $cart->getProductid(), $comparison);
